...
zokou({ nomCom: "hacker",
    ...
    if (!arg || arg == "") {
        repondre("*__Exemple : * " + prefixe + "hacker Jovince");
        return;
    }
    try {
      ...
      //  let lien = "https://e1.yotools.net" + res;
        ...
        await zk.sendMessage(origineMessage, { image: { url:anu.image}, caption: "* \t Logo by 조빈스🌸*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "dragonball", categorie: "Logo", reaction: "🐉" }, async (dest, zk, commandeOptions) => {
    ...
    try {
       ...
       // var idImg = Object.values(imgInfo)[3];
        ...
        await zk.sendMessage(dest, { image: { url: imgInfo.image }, caption: "* \t Logo by 조빈스🌸*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({ nomCom: "naruto", categorie: "Logo", reaction: "⛩" }, async (dest, zk, commandeOptions) => {
    ...
    try {
        if (!arg || arg == '') {
            repondre("*_Exemple : * " + prefixe + "naruto Jovince");
            return;
        }
        ...
        await zk.sendMessage(dest, { image: { url: img.image }, caption: "\t\t *Logo by 조빈스🌸*" }, { quoted: ms });
    }
    catch (e) {
        repondre("🥵🥵 " + e);
    }
...
zokou({nomCom: "neonlight", categorie: "Logo", reaction: "💡"}, async (dest, zk, commandeOptions) => {
  ...
  try{
    ...
    /*.then((data) => {*/
     zk.sendMessage(dest, { image: { url: maker.image }, caption: 'Logo fait par 조빈스🌸' }, { quoted: ms });
...
repondre("  Traitement en cour ...")
         await zk.sendMessage(dest,{image:{url:img.image},caption:"\t *Logo by 조빈스🌸*"})
...
zokou({nomCom:"purple",categorie:"Logo",reaction:"🧳"},async(dest,zk,commandeOptions)=>{
  var {ms,repondre,prefixe,arg}=commandeOptions;
    ...
    try{
      if(!arg||arg=="")
  {
    repondre(prefixe+"purple Jovince");return;}
...
zokou({nomCom:"gold",categorie:"Logo",reaction:"🧚🏿‍♀️"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"gold Jovince");return;
      }
...
zokou({nomCom:"arena",categorie:"Logo",reaction:"🥵"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"arena Jovince");return;
      }
...
zokou({nomCom:"incandescent",categorie:"Logo",reaction:"😋"},async(dest,zk,commandeOptions)=>{
  ...
  try{
      if(!arg||arg=="")
      {
        repondre(prefixe+"incandescent Jovince");return;
      }
